...
| TRue => true
| EQ_B  b1 b2 => true
| EQ_M t1 t2 => andb (nodec_msg n t1) (nodec_msg n t2)
| if_then_else_B t1 t2 t3 => andb (nodec_Bool n t1) (andb (nodec_Bool n t2) (nodec_Bool n t3))
| EQL t1 t2 =>  andb (nodec_msg n t1) (nodec_msg n t2)
| ver t1 t2 t3 => andb (nodec_msg n t1) (andb (nodec_msg n t2) (nodec_msg n t3))
end
with nodec_msg (n : nat ) (t:message) : bool := 
 match t with 
| if_then_else_M b3 t1 t2 =>andb (nodec_Bool n b3) (andb (nodec_msg n t1) (nodec_msg n t2))
| (Mvar n') => true
| O => true
| acc => true
| N n'=> true
| rr t1 => (nodec_msg n t1)
|new => true
| exp t1 t2 t3 => andb (nodec_msg n t1) (andb (nodec_msg n t2) (nodec_msg n t3))
| pair t1 t2 => andb (nodec_msg n t1) (nodec_msg n t2)
...
| k t1 => (nodec_msg n t1)
| to t1 => (nodec_msg n t1)
(*
| pk t1 => nodec_msg n t1
| sk t1 => notoccur_msg n t1 *)
| pi1 t1 => (nodec_msg n t1)
| pi2 t1 => (nodec_msg n t1)
| ggen t1 => (nodec_msg n t1)
| act t1 => (nodec_msg n t1)
| nonce t1 => (nodec_msg n t1)
| m t1 => (nodec_msg n t1)
| sign t1 t2 => andb (nodec_msg n t1) (nodec_msg n t2)
| reveal t1 => (nodec_msg n t1)
| i n' => true
| L t1 => (nodec_msg n t1)
| rs t1 => (nodec_msg n t1)
| f  l => (@forallb message (nodec_msg n ) l)
 end.

Eval compute in (nodec_msg 1 (dec (N 1) (N 8))).
...
Eval compute in (nodeclist_mlist [1;3] [pair ((N 5)) (sk 9); pair ((sk 3)) (pk 7)] ).

Eval compute in (nodec_mlist 8  [dec (N 1) (sk 8); dec (N 3) (r 4)] ).
...
Eval compute in (nodeclist_mlist [1;6] [dec (k (N 5)) (sk 6); dec (k (sk 9)) (pk 7)] ).


Eval compute in (decocclist_mylist [1] [msg (pair (pi2 (dec (f [(N 1);(N 2);(N 3); dec  (sk 1) O; (pk 1);(pk 2); (pk 3); (enc ( pair (pk 3) (N 6)) (r 1) (pk 2) ) ]) (sk 1))) (N 6)); msg (pair (N 6) (N 6)); msg (enc (pair (N 6) (N 6)) (r 4) (sk 2))]).
